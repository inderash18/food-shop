...
/**
 * Computes a secure SHA-256 hash of the OTP token combined with a mobile number salt.
 */
function hashOtp(mobileNumber: string, otp: string): string {
  const secretSalt = process.env.JWT_ACCESS_SECRET || 'otp-salt-key';
  return createHash('sha256')
    .update(`${mobileNumber}:${otp}:${secretSalt}`)
    .digest('hex');
}

export async function requestOtp(rawPhone: string, purpose: 'register' | 'login' | 'admin' = 'login') {
  const mobileNumber = normalizeIndianMobile(rawPhone);
  if (!mobileNumber) {
  ...
  const existing = await OtpToken.findOne({ mobileNumber, purpose, isVerified: false });
  if (existing) {
  ...
  const otpHash = hashOtp(mobileNumber, otpCode);
  ...
  if (process.env.NODE_ENV !== 'production') {
    logger.info(`[DEV OTP GENERATED] Phone: ${mobileNumber} | Code: ${otpCode}`);
  }

  // Send OTP via SMS service (non-blocking errors)
  ...
  await sendSMS({ to: mobileNumber, message: smsMessage });
...
export async function verifyOtpCode(rawPhone: string, rawOtp: string, purpose: 'register' | 'login' | 'admin' = 'login') {
  const mobileNumber = normalizeIndianMobile(rawPhone);
  if (!mobileNumber) {
    throw new AppError(400, 'INVALID_PHONE', 'Please provide a valid 10-digit Indian mobile number');
  }
  ...
  const record = await OtpToken.findOne({ mobileNumber, purpose, isVerified: false });
  if (!record) {
  ...
  const inputHash = hashOtp(mobileNumber, cleanOtp);
